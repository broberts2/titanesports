        ...
        return (
            <Style.Component>
                ...
                <Style.ScrollItemsWrapper>
                    <Style.ScrollItems>
                        {this.props.items.map(obj => <Style.ScrollItemsItem isSelected={obj.id === this.state.selectedId} onClick={() => { 
                            this.setState({ selectedId: obj.id });
                            this.props.onSelect(obj.id)
                        }}>{obj.name}</Style.ScrollItemsItem>)}
                    </Style.ScrollItems>
                </Style.ScrollItemsWrapper>
            </Style.Component>
        )
    ...
    render() {
        return (
            <Style.Component>
                ...
                />
                {this.props.imgSrc ? <Style.UrlImageImage>
                    <Img src={this.props.imgSrc} />
                </Style.UrlImageImage> : null}
            </Style.Component>
        )
    }
}
...
class TimeInput extends React.Component {
    ...
    parse(value) {
        const al = () => alert("Format is invalid. You must specify an integer, followed by a single character of 's', 'm', 'h', or 'd'.");
        const ala = () => alert("Format is valid!");
        if(value && value.length > 0) {
            const context = value.slice(-1);
            let time = value.slice(0, value.length - 1);
            if(Number.isInteger(parseInt(time)) && context.length === 1 && isNaN(context)) {
                time = parseInt(time);
                switch(context) {
                    case "s":
                        ala();
        ...
        return (
            <Style.Component>
                <TextField
                    ...
                    placeholder={"Duration (Example - 60s, 60m, 60h, 60d) | Press Enter to Verify"}
        ...
        return (
            <Style.Component>
                <TextArea
                    ...
                    height={"25px"}
                    theme={this.props.STATE.THEME}
                    controls={this.props.isLast ? [
                        { name: "Remove", operation: () => this.props.answerArray.length > 1 ? this.props.setAnswerArray(this.props.answerArray.slice(0, -1)) : null },
                        { name: "Add", operation: () => this.props.setAnswerArray(this.props.answerArray.concat("")) }
                    ] : []}
