					...
					<Utils.Document
						...
						onCreate={
							state.teams[state.team].isNew
								? async () => {
										const response =
											await GlobalActionsLeagueOfLegends.Requests.createTeam(
						...
						onSubmit={
							!state.teams[state.team].isNew
								? async () => {
										const response =
											await GlobalActionsLeagueOfLegends.Requests.updateTeam(
												Object.assign(state.teams[state.team], {
													league: state.radioValue,
													subsIds: Object.values(state.subs).filter((el) =>
														el && el.length > 0 ? el : null
						...
						validate={() =>
							state.teams[state.team].name &&
							state.teams[state.team].name.length > 0
						}
					>
						<div>
							...
							<div className={classes.roster}>
								...
								<Components.Transferlist
									...
									itemsLeft={state.itemsLeft
										.filter(
											(el) =>
												!state.players.fwd[el][
													state.radioValue === "Divinity League"
														? "divinityTeamId"
														: "conquerorTeamId"
												]
										)
										.map((el) => state.players.fwd[el].displayname)}
