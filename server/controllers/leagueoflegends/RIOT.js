...
module.exports = {
  ...
  verifyBySummonerId: async (req) => {
    const acct = await fetch(
      `https://na1.api.riotgames.com/lol/summoner/v4/summoners/${req.query.id}?api_key=${config.riotGeneralApiKey}`
    ).then((res) => res.json());
    return acct;
  },
  verifyBySummonerName: async (req) => {
    const acct = await fetch(
      `https://na1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${req.query.name}?api_key=${config.riotGeneralApiKey}`
    ).then((res) => res.json());
    return acct;
  },
